...
class PositionGetter:
    ...
    def __call__(self, batch_size: int, height: int, width: int, device: torch.device) -> torch.Tensor:
        ...
        """
        cache_key_if_static = (height, width) if isinstance(height, int) and isinstance(width, int) else None

        if cache_key_if_static is None:
            y_coords = torch.arange(height, device=device)
            x_coords = torch.arange(width, device=device)
            positions = torch.cartesian_prod(y_coords, x_coords)
            return positions.view(1, height * width, 2).expand(batch_size, -1, -1).clone()

        if cache_key_if_static not in self.position_cache:
            y_coords = torch.arange(height, device=device)
            x_coords = torch.arange(width, device=device)
            self.position_cache[cache_key_if_static] = torch.cartesian_prod(y_coords, x_coords)

        positions = self.position_cache[cache_key_if_static]
        return positions.view(1, height * width, 2).expand(batch_size, -1, -1).clone()
...
class RotaryPositionEmbedding2D(nn.Module):
    ...
    def _compute_frequency_components(
        ...
        """
        cache_key_if_static = (dim, seq_len, device, dtype) if isinstance(dim, int) and isinstance(seq_len, int) else None
        ...
        if cache_key_if_static not in self.frequency_cache:
            # Compute frequency bands
            exponents = torch.arange(0, dim, 2, device=device).float() / dim
            inv_freq = 1.0 / (self.base_frequency**exponents)
            ...
            cos_components = angles.cos().to(dtype)
            sin_components = angles.sin().to(dtype)
            self.frequency_cache[cache_key_if_static] = (cos_components, sin_components)

        return self.frequency_cache[cache_key_if_static]
